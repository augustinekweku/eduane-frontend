...
<section style="margin: 0 auto;">
        ...
        <div class="flex justify-center">
            <div class="border-r-2 border-black mr-4 p-2">
                <div class="text-center">
                    <h1 class="mb-2 text-xs sm:text-base md:text-2xl lg:text-3xl"><span>≤</span>{{$mealplan[0]->calories}}kcal</h1>
                    <h2 class="text-sm">Calorie</h2>
                </div>
            </div>
            <div class="border-r-2 border-black mr-4 p-2">
                <div class="text-center">
                    <h1 class="mb-2 text-xs sm:text-base md:text-2xl lg:text-3xl">{{$mealplan[0]->fat}}%</h1>
                    <h2 class="text-sm">Fat</h2>
                </div>
            </div>
            <div class="border-r-2 border-black mr-4 p-2">
                <div class="text-center">
                    <h1 class="mb-2 text-xs sm:text-base md:text-2xl lg:text-3xl">{{$mealplan[0]->protein}}%</h1>
                    <h2 class="text-sm">Protein</h2>
                </div>
            </div>
            <div class=" mr-4 p-2">
                <div class="text-center">
                    <h1 class="mb-2 text-xs sm:text-base md:text-2xl lg:text-3xl">{{$mealplan[0]->carbs}}%</h1>
                    <h2 class="text-sm lg:text-base">Carbs</h2>
                </div>
            </div>
    ...
    <section class="plan_durations w-full sm:w-full md:w-4/5 lg:w-3/5 mx-auto">
                ...
                @if (count($mealplandurations)> 0)
                    @foreach ($mealplandurations as $mealplanduration)
                    <div class="flex flex-wrap py-4 my-3  bg-gray-100 rounded-md shadow-md">
                        <div class="w-full flex justify-center items-center md:w-2/5 lg:w-2/5 ">
                            <h1 class="text-base ">{{$mealplanduration->title}}</h1>
                        </div>
                        <div class=" w-full md:w-2/5 lg:w-2/5 ">
                          <p class=" text-xs sm:text-sm md:text-base p-1 py-2 sm:py-1 text-center sm:text-left">
                            {{$mealplanduration->description}}
                          </p>
                        </div>
                        <div class=" w-full flex justify-center items-center md:w-1/5 lg:w-1/5">
                          <a  class="button bg-orange text-white rounded-3xl py-1 px-4">  Select</a>
                        </div>
                      </div>                
                    @endforeach
                @endif
    ...
    <section class="meal_packages w-full sm:w-full md:w-4/5 lg:w-3/5 mx-auto py-2">
        ...
        <div class="package_block bg-gray-100 rounded-md shadow-md  my-2">
            <h1 class="text-center text-base py-2"> {{$mealplanduration->title}} </h1>
            <div class="flex w-2/3  mx-auto package_item ">
                <div class="flex p-2 justify-center items-center">
                    <div class="package_image">
                        <img class="h-32 sm:h-32 w-32" src="img/meal_plan.webp" alt=" get-lean">
                    </div>
